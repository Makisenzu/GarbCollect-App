...
const CalendarWidget = ({ schedules }) => {
    ...
    const getDayColor = (schedules) => {
        ...
        if (hasActive) return 'bg-yellow-100 border border-yellow-300';
    ...
    const getStatusDot = (schedules) => {
        ...
        if (hasActive) return 'bg-yellow-500';
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
        days.push(<div key={`empty-${i}`} className="h-12 w-8"></div>);
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        days.push(
            <div 
                key={day} 
                className={`h-12 w-8 flex flex-col items-center justify-start p-1 rounded relative ${
                    isCurrentDay 
                        ? 'bg-blue-600 text-white' 
                        : dayColor || 'text-gray-700 hover:bg-gray-100'
                }`}
            >
                <span className={`text-xs font-medium ${isCurrentDay ? 'text-white' : 'text-gray-700'}`}>
                    {day}
                </span>
                {statusDot && (
